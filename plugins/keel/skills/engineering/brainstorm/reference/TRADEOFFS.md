...
Write to `$KEEL_HOME/repos/$REPO_KEY/runs/<slug>/brainstorm.md`:
...
## Recommendation
<chosen option + why> → hand to /keel:plan to expand into a full plan.
````

`brainstorm` diverges (many approaches, breadth research, never committed); `plan` converges (one approach,
depth research, code examples, task seed). Compose them: brainstorm → pick winner → `/keel:plan` on it.

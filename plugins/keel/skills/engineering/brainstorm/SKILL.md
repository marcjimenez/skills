...
1. **Understand the problem.** If the ask is fuzzy, invoke `/keel:requirements` to pin the constraints
   enough to compare options (a full spec is not required at this stage).

2. **Research (breadth).** Invoke `/keel:research` in `breadth` mode — one shallow survey pass: what
...
Write to `$KEEL_HOME/repos/$REPO_KEY/runs/<slug>/brainstorm.md` (never inside the target repo).
...
> Recommended: Option <X>. Run `/keel:plan` to expand it into a full plan.

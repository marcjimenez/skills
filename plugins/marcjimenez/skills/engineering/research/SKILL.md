...
feeds `/marcjimenez:plan` and `/marcjimenez:brainstorm`. Two modes (the caller passes one):
...
`/marcjimenez:setup` `reference/CONFIG-SCHEMA.md`). Use ONLY connections marked `enabled`; if one is off, fall back
...
1. **Reuse hunt FIRST (ladder rungs 1-5).** The laziest research result is "we already have it." Apply the
   Climb-the-Ladder doctrine — see `/marcjimenez:reuse`. grep the repo (`shared`/`utils`/`common`/`lib`, siblings);

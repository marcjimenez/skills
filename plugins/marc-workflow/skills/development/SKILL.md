...
> You are reviewing this diff for over-engineering and missed reuse. Check: abstractions that serve one caller, utilities that already exist elsewhere in the repo (search the codebase to prove duplication), dead code introduced, import boundary violations, unnecessary indirection layers, things that could be 5 lines but are 50. For each "already exists" claim, cite the existing utility's file:line.
